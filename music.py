...
root.title("Nitin's Music Player")
root.resizable(0, 0)
def play_song(song_name: StringVar, songs_list: Listbox, status: StringVar):
    song_name.set(songs_list.get(ACTIVE))
    mixer.music.load(songs_list.get(ACTIVE))
    mixer.music.play()
    status.set("Song Running")

def stop_song(status: StringVar):
    mixer.music.stop()
    status.set("Song Cancelled")

def load(listbox):
    os.chdir(filedialog.askdirectory(title='Open your music folder'))
    tracks = os.listdir()
    for track in tracks:
...
song_frame = LabelFrame(root, text='Audio Box', bg='lightPink', width=400, height=80)
song_frame.place(x=0, y=0)

button_frame = LabelFrame(root, text='Features', bg='lightPink', width=400, height=120)
button_frame.place(y=80)

listbox_frame = LabelFrame(root, text='Your Playlist', bg='lightPink')
listbox_frame.place(x=400, y=0, height=200, width=300)

current_song = StringVar(root, value='Select a song from the Playlist')
...
Label(song_frame, text='Song Name :', bg='lightPink', font=('Times', 10, 'bold')).place(x=20, y=20)
